...
function validate(validatableInput: IValidatble) {
  let isValid = true;
...
class ProjectInput extends Component<HTMLDivElement, HTMLFormElement> {
  ...
  private gatherUserInput(): [string, string, number] | void {
    ...
    const titleValidatable: IValidatble = {
      value: enteredTitle,
      required: true,
      minLength: 2,
      maxLength: 50
    };
    const descriptionValidatable: IValidatble = {
      value: enteredDescription,
      required: true,
      minLength: 5,
      maxLength: 255
    };
    const peopleValidatable: IValidatble = {
      value: enteredPeople,
      required: true,
      min: 1,
      max: 5
